...
boru-driven command-line client **and** interactive REPL for the Geonode
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/geonode-cli)
make build
...
./geonode-cli list proxy

# 5. Override the API base URL for a single call
GEONODE_BASE=https://api.example.com ./geonode-cli list proxy
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/geonode-cli
   ```
...
Run with no arguments to open a REPL (prompt `geonode>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `proxy`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/geonode-cli`. |
...
(`./geonode-cli <expr>`) and interactively (the REPL), and expressions compose the
