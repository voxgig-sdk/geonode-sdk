// Command Geonode-cli is a CLI + REPL for the Geonode SDK,
...
const prompt = "geonode"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "proxy"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      GEONODE_APIKEY (api key), GEONODE_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: GEONODE_APIKEY carries the API key and
	// GEONODE_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("GEONODE_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("GEONODE_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewGeonodeSDK(opts)
