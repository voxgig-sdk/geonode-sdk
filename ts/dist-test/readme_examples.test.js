...
const TEST_SEED = { "entity": { "proxy": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+GeonodeSDK\b/.test(code) ||
        /\bGeonodeSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+GeonodeSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/GeonodeSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
