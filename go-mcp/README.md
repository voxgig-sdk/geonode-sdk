...
[MCP](https://modelcontextprotocol.io) server exposing the Geonode SDK as
two agent tools — `geonode_list` and `geonode_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/geonode-mcp)
make build

# 2. Provide credentials via the environment
export GEONODE_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user geonode \
  -- /absolute/path/to/geonode-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./geonode-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// geonode_list: first page of records
{ "entity": "proxy" }
{ "entity": "proxy", "query": { } }

// geonode_load: one record by id
{ "entity": "proxy", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/geonode-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user geonode \
     -- "$PWD"/dist/*/geonode-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `geonode_list` and `geonode_load` tools now appear
   in new sessions. Ask the agent to *"list proxy using geonode"*
   and it calls `geonode_list` with `{"entity":"proxy"}`.
...
```sh
export GEONODE_APIKEY=sk_live_xxx            # API key
export GEONODE_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `geonode_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `geonode_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `GEONODE_APIKEY` | API key sent with every request. |
| `GEONODE_BASE` | Optional override of the API base URL. |
...
```sh
./geonode-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"geonode_load","arguments":{"entity":"proxy","query":{"id":1}}}}'
```
