...
module GeonodeUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = GeonodeContext.new({}, nil)
    end
    op = ctx.op || GeonodeOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || GeonodeResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(GeonodeError) ? err.msg : err.to_s
    ...
    sdk_err = GeonodeError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(GeonodeError)
