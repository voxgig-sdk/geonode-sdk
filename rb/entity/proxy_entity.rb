...
class ProxyEntity
  ...
  def data_set(args)
    if args
      @_data = GeonodeHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = GeonodeHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Array<Proxy>, Array] the matching Proxy items; raises GeonodeError on failure
