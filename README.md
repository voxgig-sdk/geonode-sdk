...
Geonode API client, generated from the OpenAPI spec.

The Geonode API enables ethical data collection for the modern web, allowing developers to utilize scalable proxies and APIs for any target while offering expert support throughout the process.

Learn more about Geonode API at [github.com/l0v3m0n3y/geonode](https://github.com/l0v3m0n3y/geonode).

This is an unofficial SDK for the Geonode public API, generated by Voxgig with [`@voxgig/sdkgen`](https://github.com/voxgig/sdkgen). It is not affiliated with, endorsed by, or sponsored by the upstream API provider.
...
This SDK exposes the API as a small set of **semantic entities** — Proxy — that you
...
const items = await client.Proxy().list()
```
...
```bash
cd go-mcp && go build -o geonode-mcp .
```
...
```json
{
  "mcpServers": {
    "geonode": {
      "command": "/abs/path/to/geonode-mcp"
    }
  }
}
```
...
| --- | --- | --- |
| **Proxy** | The Proxy entity (list). | `/proxy-list` |
...
# List all proxys (returns a list, raises on error)
proxys = client.Proxy().list()
for proxy in proxys:
    print(proxy)
```
...
// List all proxys (returns an array; throws on error)
$proxys = $client->Proxy()->list();
print_r(array_map(fn($item) => $item->data_get(), $proxys));
```

### Golang

```go
import sdk "github.com/voxgig-sdk/geonode-sdk/go"
...
# List all proxys (returns an Array; raises on error)
proxys = client.Proxy.list
puts proxys
```
...
Generated from the Geonode API OpenAPI spec by [@voxgig/sdkgen](https://github.com/voxgig/sdkgen).
