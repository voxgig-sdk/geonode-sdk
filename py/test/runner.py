# Geonode SDK test runner

from __future__ import annotations
import os
import json

from geonode_sdk.utility.voxgig_struct import voxgig_struct as vs


class GeonodeTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            GeonodeTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = GeonodeTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = GeonodeTestRunner.getenv("GEONODE_TEST_LIVE")
        override = GeonodeTestRunner.getenv("GEONODE_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = GeonodeTestRunner.getenv(key)
        ...
        explain = GeonodeTestRunner.getenv("GEONODE_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["GEONODE_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if GeonodeTestRunner._test_control is not None:
            return GeonodeTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                GeonodeTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            GeonodeTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = GeonodeTestRunner.load_test_control()
...
def load_env_local():
    GeonodeTestRunner.load_env_local()


def env_override(m):
    return GeonodeTestRunner.env_override(m)


def entity_data(v):
    return GeonodeTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return GeonodeTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return GeonodeTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return GeonodeTestRunner.load_test_control()


def live_client_options():
    return GeonodeTestRunner.live_client_options()


def live_delay_ms():
    return GeonodeTestRunner.live_delay_ms()
