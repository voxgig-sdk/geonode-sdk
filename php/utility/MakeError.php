...
class GeonodeMakeError
{
    public static function call(?GeonodeContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new GeonodeContext([], null);
        }
        $op = $ctx->op ?? new GeonodeOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new GeonodeResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof GeonodeError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new GeonodeError('', $msg, $ctx);
        ...
        if ($err instanceof GeonodeError) {
