...
class ProxyEntity
{
    ...
    {
        if ($args) {
            $this->_data = GeonodeHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = GeonodeHelpers::to_map(Struct::clone($args)) ?? [];
     ...
     *   the SDK boundary; throws GeonodeError on failure (item-5 convention).
     */
