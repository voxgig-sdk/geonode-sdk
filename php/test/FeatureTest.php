...
class FtCtrl extends GeonodeControl
{
...
class FtClient
{
    ...
    private GeonodeUtility $utility;

    public function __construct(GeonodeUtility $utility, array $options)
    {
    ...
    public function get_utility(): GeonodeUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (GeonodeContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (GeonodeContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = GeonodeConfig::make_feature($name);
    ...
    private static function buildUrl(GeonodeSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof GeonodeError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new GeonodeResponse($fetched);
        }
    ...
    private function fail(GeonodeContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (GeonodeContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
