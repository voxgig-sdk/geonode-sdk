...
class GeonodeSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = GeonodeHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = GeonodeHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!GeonodeFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, GeonodeFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return GeonodeUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = GeonodeHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = GeonodeHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = GeonodeHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new GeonodeError($op . "_allow",
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = GeonodeHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new GeonodeError("graphql_error",
                "GeonodeSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
