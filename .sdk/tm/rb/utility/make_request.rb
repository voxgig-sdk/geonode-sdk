...
module GeonodeUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = GeonodeResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = GeonodeResponse.new(fetched)
    else
