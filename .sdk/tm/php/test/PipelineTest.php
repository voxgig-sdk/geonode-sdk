...
{
    private static function utility(): GeonodeUtility
    {
        return new GeonodeUtility();
    }
    ...
    private static function ctx(array $over = []): GeonodeContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new GeonodeContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new GeonodeOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof GeonodeError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new GeonodeOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = GeonodeMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new GeonodeOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = GeonodeMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new GeonodeOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = GeonodeMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new GeonodeSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = GeonodeMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new GeonodeResponse([]),
            'result' => new GeonodeResult([]),
        ]);
        [, $err] = GeonodeMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new GeonodeSpec([]),
            'response' => null,
            'result' => new GeonodeResult([]),
        ]);
        [, $err] = GeonodeMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new GeonodeSpec(['step' => 's']),
            'response' => new GeonodeResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new GeonodeResult([]),
        ]);
        [, $err] = GeonodeMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new GeonodeSpec(['step' => 's']),
            'response' => new GeonodeResponse(self::resp(200, ['v' => 1])),
            'result' => new GeonodeResult([]),
        ]);
        [, $err] = GeonodeMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new GeonodeSpec(['step' => 's']),
            'response' => new GeonodeResponse(self::resp(200, ['v' => 2])),
            'result' => new GeonodeResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        GeonodeMakeResponse::call($ctx);
    ...
    {
        $preset = new GeonodeResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new GeonodeSpec([]),
            'response' => new GeonodeResponse([]),
            'result' => new GeonodeResult([]),
        ...
        [$got, $err] = GeonodeMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new GeonodeResult([])]);
        [, $err] = GeonodeMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new GeonodeSpec([]), 'result' => null]);
        [, $err] = GeonodeMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new GeonodeSpec(['step' => 's']),
            'result' => new GeonodeResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new GeonodeOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = GeonodeMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new GeonodeSpec(['step' => 's']),
            'result' => new GeonodeResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new GeonodeOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = GeonodeMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new GeonodeResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new GeonodeSpec([]),
            'result' => new GeonodeResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = GeonodeMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = GeonodeMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new GeonodeError('boom', 'boom');
        $utility->fetcher = function (GeonodeContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new GeonodeSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = GeonodeMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (GeonodeContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = GeonodeMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(GeonodeResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (GeonodeContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new GeonodeSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = GeonodeMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new GeonodeResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new GeonodeSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = GeonodeMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = GeonodeMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = GeonodeMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new GeonodeResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, GeonodeDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new GeonodeResult(['ok' => false])]);
        $this->expectException(GeonodeError::class);
        GeonodeDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new GeonodeResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', GeonodeMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        GeonodeMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new GeonodeBaseFeature();
        $b = new GeonodeBaseFeature();
        GeonodeFeatureAdd::call($ctx, $a);
        GeonodeFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): GeonodeBaseFeature
    {
        $f = new GeonodeBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        GeonodeFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): GeonodeContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new GeonodeSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = GeonodePrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new GeonodeResponse(['headers' => null]),
            'result' => new GeonodeResult([]),
        ]);
        GeonodeResultHeaders::call($ctx);
