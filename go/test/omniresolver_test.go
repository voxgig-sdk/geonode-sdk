...
import (
	...
	omni "github.com/voxgig-sdk/geonode-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.GeonodeError); is {
				out := map[string]any{
					"name":    "GeonodeError",
...
func omniCtx(arg any, client *sdk.GeonodeSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.GeonodeSDK); is {
			client = live
